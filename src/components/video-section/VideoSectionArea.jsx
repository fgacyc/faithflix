...
export default function VideoSectionArea({ classNames, sectionTitle }) {
  ...
  const [currentViewPortElementIndex, setCurrentViewPortElementIndex] =
    useState(0);
  ...
  function handleScroll(direction) {
    ...
    if (direction === "right") {
      ...
      for (let i = currentViewPortElementIndex; i >= 0; i--) {
        if (!isInViewport(children[i])) {
          firstUnseenChild = children[i];
          if (i === currentViewPortElementIndex || i === children.length - 1)
            continue;
          setCurrentViewPortElementIndex(i);
          break;
        }
    ...
    children[currentViewPortElementIndex].scrollIntoView({
      behavior: "smooth",
      block: "nearest",
      inline: "start",
    });
  ...
  return (
    <div className={`${classNames}`}>
      <div
        className={`text-white font-bold lg:text-xl md:text-lg sm:text-base text-base mb-2 
      ...
      >
        ...
        <span
          className={`inline-block absolute right-[-100%]
      ...
      <div
        className={
          "overflow-x-auto w-full flex flex-nowrap flex-row no-scrollbar relative"
        }
        ref={areaRef}
      >
        <div
      ...
      <div
        className={`absolute  w-[50px]  z-10 flex bottom-0 left-0 scroll-smooth
