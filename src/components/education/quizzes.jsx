  ...
  useEffect(() => {
    ...
    async function getClassData() {
      ...
      const data = await fetch(
        `${process.env.NEXT_PUBLIC_CMS_HOST_URL}/api/classes/${classID}?populate=*`,
        options
      );
        ...
        <div className={"bg-transparent w-full"} key={key}>
          ...
          {showResult && allQuizzes && (
            <div className={"text-white w-full flex flex-col items-center"}>
              ...
              >
                <CardBody className="justify-center items-center pb-0">
                  <CircularProgress
                    ...
                    value={(correctAnswersNum / allQuizzes.length) * 100}
                    strokeWidth={4}
                    showValueLabel={true}
                    color="success"
                    aria-label="process"
              ...
              <div className={"mt-6"}>
                {passed ? (
                  <div className={"text-green-400"}>YOU PASSED THE QUIZ</div>
                ) : (
