...
import HeaderNotificationIcon from "@/components/header/header-notificationIcon";
import { useRouter } from "next/router";
import HeaderGenre from "@/components/header/header-genre";
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={
            "flex flex-row justify-between items-center w-[calc(155px + 270px)]"
          }
        >
          {!avatarAndLogoOnly && (
            <HeaderSearch
          ...
          <div className={`flex flex-row justify-start items-center w-[110px]`}>
            <HeaderNotificationIcon
              className={`${avatarAndLogoOnly ? "invisible" : ""}`}
            />
            <HeaderAvatar />
          </div>
        </div>
      </header>
      {currentTabIndex !== 0 && <div className={"h-[65px]"}></div>}
    </>
  );
}
