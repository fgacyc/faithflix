...
export default function HeaderAvatar() {
  ...
  const [picture, setPicture] = useUserStore((state) => [
    state.picture,
    state.setPicture,
  ]);
  const setName = useUserStore((state) => state.setName);
  const setLanguage = useUserStore((state) => state.setLanguage);
  const [oauth2_id, setOauth2_id] = useUserStore((state) => [
    state.oauth2_id,
    state.setOauth2_id,
  ]);
  ...
  return (
    ...
    >
      <PopoverTrigger>
        <div
          className="flex flex-row items-center justify-between gap-3 w-[60px]  cursor-pointer ml-4"
        ...
        >
          ...
          <span
            className={`border-x-5 border-t-5 border-x-transparent border-t-white border-solid
                  h-0 w-0 m-0 transition transform duration-300 ease-in-out ${
      ...
      <PopoverContent
        className={"rounded bg-[#080605] text-white p-0"}
        onMouseEnter={mouseEnter}
        onMouseLeave={mouseLeave}
      >
        <div className="p-0 text-xs">
          ...
          <div className="ml-2 mb-1  cursor-pointer">
            <BsClockHistory
              className={"text-[#B3B3B3] text-2xl inline-block m-1"}
            />
            <span
              className={"ml-2 hover:underline"}
